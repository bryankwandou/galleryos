...
function GalleryManager({ images, onPublish, notice, anchorUrl }: { images: StudioImage[]; onPublish: () => void; notice: string; anchorUrl: string }) {
  ...
  return <main className="studio-content"><div className="page-title compact"><div><span>Harper & Chen · Draft</span><h1>Gallery builder</h1><p>Review the client-facing sequence before publishing.</p></div><button className="studio-button" onClick={onPublish}>Publish with proof <ArrowUpRight size={15} /></button></div>{notice && <div className="notice"><Check size={16} />{notice}{anchorUrl && <a href={anchorUrl} target="_blank" rel="noreferrer">Solana Explorer</a>}</div>}<div className="builder"><aside><h3>Gallery details</h3><label>Gallery title<input defaultValue="Harper & Chen" /></label><label>Subtitle<input defaultValue="New York · July 2026" /></label><label>Download access<select defaultValue="14"><option value="7">7 days</option><option value="14">14 days</option><option value="30">30 days</option></select></label><div className="secure-note"><LockKeyhole size={15} />Private token access</div></aside><section><div className="client-preview-head"><div className="monogram">H+C</div><div><span>North & River</span><h2>Harper & Chen</h2><p>New York · July 2026</p></div><Link href="/gallery/harper-chen" target="_blank">Open client view <ArrowUpRight size={14} /></Link></div><div className="builder-grid">{included.map(image => <div key={image.id} style={{backgroundImage:`url(${image.url})`}} />)}</div></section></div></main>;
}
